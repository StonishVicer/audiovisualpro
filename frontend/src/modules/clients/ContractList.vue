...
const createContrato = async () => {
    if (!id_proyecto_seleccionado.value || !id_cliente_seleccionado.value || !fecha_firma.value || !monto_contrato.value) {
        error.value = true
        return
    }
    ...
    try {
        ...
        displayToast('Contrato creado exitosamente', 'success')
        showModal.value = false
        limpiarCampos()
        getContratos()
    } catch (err) {
        console.error("Error creando contrato:", err)
        displayToast('Error al crear el contrato', 'error')
    } finally {
...
const deleteContrato = async (id) => {
    ...
    try {
        ...
        displayToast('Contrato eliminado', 'success')
        getContratos()
    } catch (err) {
        console.error("Error eliminando contrato:", err)
        displayToast('Error al eliminar el contrato', 'error')
    }
}
</script>
...
<template>
    <div class="h-screen flex flex-col">
                    ...
                    <tbody>
                        <tr v-for="contrato in contratos" :key="contrato.id_contrato"
                            ...
                            <td class="px-4 py-3 flex justify-center gap-2">
                                ...
                                <button title="Eliminar" @click="deleteContrato(contrato.id_contrato)"
        ...
        <Modal :show="showModal" @close="showModal = false" title="Registrar Nuevo Contrato" size="lg">
            <div>
